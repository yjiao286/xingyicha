...
def _safe_save(file_storage, prefix=''):
    ...
    if not base or not base.lower().endswith(('.docx', '.doc', '.pdf')):
        return None, None
...
def _is_person_name(name):
    ...
    if name_stripped in ('姓名', '职务', '签字', '盖章', '授权', '电话', '地址', '传真',
...
def _build_normalized_map(text):
    """Return (norm_text, positions) where norm_text has whitespace
    collapsed and positions[i] is the original index of norm_text[i]."""
    norm_chars = []
    positions = []
...
def find_common_segments(text1, text2, min_len=15):
    ...
    Uses SequenceMatcher (C implementation) on whitespace-normalised texts so
    matches are insensitive to spacing differences between docx and PDF
    extraction.  The minimum-meaningful-chars check uses a combined CJK+
    ASCII-letter+digit count so English/mixed technical documents get the
    same recall as Chinese-only ones.

    Returns list of (pos1, pos2, length, segment, ctx1, ctx2) — same format
    as before.
    """
    import difflib
    ...
    if len(norm1) < min_len or len(norm2) < min_len:
        return []

    matcher = difflib.SequenceMatcher(None, norm1, norm2, autojunk=False)
    blocks = matcher.get_matching_blocks()  # list of Match(a, b, size)
    # Last element is a dummy (a=len, b=len, size=0) — skip it.
    results = []
    found_positions = set()
    min_meaningful = max(5, min_len // 2)

    for block in blocks:
        if block.size < min_len:
            continue
        # Normalised segment
        seg_norm = norm1[block.a:block.a + block.size]
        if _count_meaningful_chars(seg_norm) < min_meaningful:
            continue
        # Map back to raw positions
        raw_a = pos1_map[block.a]
        raw_b = pos2_map[block.b]
        # Deduplicate overlapping matches (keep the first occurrence)
        if any(abs(raw_a - p) < 10 for p in found_positions):
            continue
        found_positions.add(raw_a)
        # Build raw-text segment (use raw text for exact context)
        seg_raw = text1[raw_a:raw_a + len(seg_norm)]
        ctx_before = 100
        ctx_after = 100
        ctx1 = text1[max(0, raw_a - ctx_before):raw_a + len(seg_norm) + ctx_after]
        ctx2 = text2[max(0, raw_b - ctx_before):raw_b + len(seg_norm) + ctx_after]
        results.append((raw_a, raw_b, len(seg_norm), seg_raw, ctx1, ctx2))
...
def analyze_stream():
    ...
    def generate():
        ...
        thread.start()
        ANALYSIS_TIMEOUT = 270  # < gunicorn timeout(300) so we can flush an error
