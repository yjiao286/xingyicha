...
### 围串标考点修复（工程类/服务类语料，2026-09-16）

对照工程类（9 考点）与服务类（9 线索）两份标注逐条比对后修复。覆盖：**工程类 3/9 → 8/9，服务类 → 9/9**（剩余考点一需读扫描图片，见下）。

**① 环境噪声过滤删掉了线索本身**（`_demote_environmental_pool_values`）：原判据「出现在 ≥min_groups 份且 ≥ratio 比例」在 3 家投标时退化为「出现在全部 3 家」——而这正是考点三（不同投标人的联系人、电话或者地址**相同**）的定义。实测三家「供应商基本情况表」共用电话 `13911112222`，被整体删除。改为**按来源判定**：值若出现在参照（招标）文件里即招标方数据，否则保留；无参照文件时才回退频次规则。

**② phones 池只收手机号**：`_iter_mobiles` 只认 `1[3-9]\d{9}`，座机从来不进跨文档比对池，服务类线索9（`010-88886666` 三维=己公司）在结构上不可能触发。新增 `_iter_landlines`（座机，要求区号分隔符；无分隔形态仅在有 电话/座机 标签时接受），`_iter_phones` = 手机+座机，管道表电话列同步。
...
**⑥ 姓名捕获**：新增 `（项目经理姓名）X` 形态（括号提示+填写值）。丙公司的**项目经理承诺书**写吴九、简历表写郑明，只有承诺书能把该文件与乙公司联系起来（考点九）；该形态出现在无区段标记的承诺书散文里，故做全文扫描。
...
实测效果：丙公司那份 169 张图命中 19 张、80s，文本 21137 → 27827 字符，**并带出了考点八的关键证据**——其保证金凭证 OCR 出的付款人是**甲公司**（凭证号/金额/用途字段齐全），即"保证金由同一单位转出"。这是纯图片线索，此前完全不可见。
