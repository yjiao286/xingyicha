...
**仍未覆盖**：考点一「甲公司和丙公司授权委托书法人均为孙七」——`孙七` 在**甲公司**的任何可提取文本中都不存在（docx/pdf 均无，页眉文本框也没有），「仅授权书文字」提示其位于授权书签章扫描图内。同理考点八（保证金图片）、服务线索6（CMMI 证书图）需要内嵌图片 OCR：一份商务标有 128 张图（合计 40MB），全量 OCR 成本过高，需按章节就近选择性 OCR。
